...
import OBSCURA_IIFE_SRC from "../../../../pkgs/Obscura/dist/obscura.iife.js?text";
		...
		return [
			...
			script(
				"data:text/javascript;charset=utf-8;base64," +
					...
					// the host's obscura.encode/decode are closures over
					// host-realm WASM memory and don't survive
					// Function.prototype.toString().
					${obscuraSrc}

					if (!globalThis.__obscura || globalThis.__obscura.ready !== true) {
						throw new Error("[scramjet-controller] Obscura failed to initialize in proxied frame; aborting bootstrap to avoid codec mismatch with host");
					}
					...
					$scramjetController.load({
						...
						codecEncode: globalThis.__obscura.encode,
						codecDecode: globalThis.__obscura.decode,
						obscuraSrc: ${JSON.stringify(obscuraSrc)},
			...
			interface: {
				...
				getWorkerInjectScripts: (meta, type, script) => {
					...
					(()=>{
						...
						// Same obscura-inlining dance as in the document
						// bootstrap path above — workers can't inherit
						// the host's WASM-backed codec via Function.toString
						// either, so we re-initialize a worker-local
						// obscura instance and use its encode/decode.
						${OBSCURA_IIFE_SRC}
						if (!globalThis.__obscura || globalThis.__obscura.ready !== true) {
							throw new Error("[scramjet-controller] Obscura failed to initialize in proxied worker; aborting bootstrap to avoid codec mismatch with host");
						}

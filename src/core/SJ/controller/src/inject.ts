...
type Init = {
	...
	yieldGetInjectScripts: (
		...
		prefix: URL,
		cookieJar: CookieJar,
		// IIFE-format Obscura bundle as raw text. Threaded through here
		// so nested-frame data URLs (generated by re-invoking
		// yieldGetInjectScripts inside the proxied frame) can splice the
		// same bundle into their own bootstrap. See
		// `src/core/SJ/controller/src/index.ts` for the host-side
		// counterpart and the rationale.
		obscuraSrc: string
	) => any;
	codecEncode: (input: string) => string;
	codecDecode: (input: string) => string;
	// Same Obscura IIFE bundle text as in the recursive
	// yieldGetInjectScripts param above; carried on the Init payload
	// so the frame's own ExecutionContextWrapper can pass it forward.
	obscuraSrc: string;
		...
		const context: ScramjetContext = {
			...
			interface: {
				getInjectScripts: this.init.yieldGetInjectScripts(
					...
					this.init.prefix,
					this.cookieJar,
					// Forward the Obscura bundle so nested frames get
					// their own WASM-backed codec too. See the matching
					// signature in `Init.yieldGetInjectScripts` above.

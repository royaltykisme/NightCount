...
class Search implements SearchInterface {
	...
	async init(searchbar: HTMLInputElement) {
		...
		const suggestionList = this.ui.createElement('div', {
			class: 'suggestion-list fixed z-[9999] left-1/2 transform w-full max-w-2xl bg-[var(--bg-2)] rounded-xl shadow-lg border border-[var(--main-35a)] backdrop-blur-sm',
			id: 'suggestion-list',
			style: 'top: 30%; transform: translate(-50%, -50%); min-height: 20vh; max-height: 40vh; overflow-y: auto; display: none;'
		});
		...
		window.addEventListener('keydown', async (event: KeyboardEvent) => {
			...
			if (event.key === 'ArrowDown') {
				...
				if (this.selectedSuggestionIndex !== -1) {
					const selectedItem =
						suggestionItems[this.selectedSuggestionIndex];
					selectedItem.click();
				} else {
					suggestionList.style.display = 'none';
					this.clearSuggestions();
					if (searchbar.value.trim()) {
						await this.handleDirectNavigation(
	...
	createSection(titleText: string, iconName?: string): Section {
		const section = this.ui.createElement(
			'div',
			{
				class: 'search-section p-4 border-b border-[var(--main-20a)] last:border-b-0',
				style: 'display: none;'
			},
			[
				this.ui.createElement(
					'div',
					{
						class: 'search-title flex items-center gap-2 mb-3 text-sm font-medium text-[var(--main)] uppercase tracking-wide'
					},
					[
				...
				this.ui.createElement('div', {
					class: 'search-results space-y-1'
				})
			]
		);
	...
	moveToPreviousSection(): void {
		...
		while (
			sectionsArray[this.currentSectionIndex].searchResults.children
				.length === 0
		) {
			this.currentSectionIndex =
				(this.currentSectionIndex - 1 + sectionsArray.length) %
	...
	moveToNextSection(): void {
		this.currentSectionIndex =
			...
			Object.values(this.sections).length;
		while (
	...
	private async syncAddressBar(
		...
		try {
			// First chance: raw path maps directly to an internal URL.
			const rawPath = new URL(iframe.src).pathname;
			const internalCheck = await this.proto.getInternalURL(rawPath);
			if (
	...
	createSuggestionItem(
		...
		const listItem = this.ui.createElement(
			'div',
			{
				class: 'flex items-center gap-3 p-3 rounded-lg hover:bg-[var(--main-20a)] hover:border-l-2 hover:border-l-[var(--main)] cursor-pointer transition-all group border-l-2 border-l-transparent',
				tabindex: '-1'
			},
			[
				this.ui.createElement('i', {
					'data-lucide':
						iconName ||
						(this.isValidUrl(suggestion)
							? 'external-link'
							: 'search'),
					class: 'w-4 h-4 text-[var(--proto)] group-hover:text-[var(--main)] transition-colors flex-shrink-0'
				}),
				this.ui.createElement(
					'div',
					{
					...
					[
						this.ui.createElement(
							'div',
							{
								class: 'suggestion-text text-sm text-[var(--text)] truncate group-hover:text-[var(--text)]'
							},
						...
						...(displayName && displayName !== suggestion
							? [
									this.ui.createElement(
										'div',
										{
											class: 'text-xs text-[var(--proto)] truncate mt-1 group-hover:text-[var(--main-70)]'
										},
										[suggestion]
									)
								]
		...
		const listItem = this.ui.createElement(
			...
			[
				this.ui.createElement(
					'div',
					{
						class: 'w-8 h-8 rounded-md overflow-hidden flex-shrink-0 bg-[var(--bg-1)] ring-1 ring-[var(--main-35a)]'
					},
					[
				...
				this.ui.createElement(
					...
					[
						this.ui.createElement(
							'div',
							{
								class: 'text-sm text-[var(--text)] font-medium truncate group-hover:text-[var(--text)]'
							},
							[game.name]
						),
						this.ui.createElement(
							'div',
							{
								class: 'text-xs text-[var(--proto)] truncate group-hover:text-[var(--main-70)]'
							},
							['Game']
						)
					]

...
export class TabLayout {
	...
	get tabContentWidths() {
		...
		const targetWidth =
			(tabsContentWidth - 2 * 9 + tabsCumulativeOverlappedWidth) /
			numberOfTabs;
		...
		for (let i = 0; i < numberOfTabs; i += 1) {
			const extraWidth =
				flooredClampedTargetWidth < 240 && extraWidthRemaining > 0
					? 1
					: 0;
	...
	get tabContentHeights() {
		...
		for (let i = 0; i < numberOfTabs; i += 1) {
			const extraHeight =
				flooredClampedTargetHeight < 36 && extraHeightRemaining > 0
					? 1
					: 0;
	...
	renderGroupHeaders = (): void => {
		const existingHeaders =
			this.tabs.el.querySelectorAll('.tab-group-header');
		...
		groupedTabs.forEach((groupTabs, groupId) => {
			...
			firstTabElement.parentNode!.insertBefore(
				groupHeader,
				firstTabElement
			);
	...
	private setupGroupHeaderContextMenu = (
					...
					onclick: () => {
						group.tabIds.forEach((tabId: string) => {
							...
							if (tab) {
								...
								// Use the bookmark UI flow (which decodes proxied URLs and
								// pops the prompt). The API-level BookmarkManager doesn't
								// expose `addBookmark` — only `createBookmark` — so the
								// previous direct call was a silent no-op.
		...
		menuItems.push(
			this.tabs.ui.createElement(
				'button',
				{
					class: 'flex items-center gap-3 px-4 py-2 hover:bg-yellow-500/10 text-yellow-400 hover:text-yellow-300 transition-colors w-full text-left text-sm rounded-md',
					onclick: () => {
			...
			this.tabs.ui.createElement(
				'button',
				{
					...
					onclick: () => {
						if (
							confirm(
								`Delete group "${group.name}" and all its tabs?`
							)
						) {
		...
		this.tabs.nightmarePlugins.rightclickmenu.attachTo(
			headerElement,
			() => {
				return menu;
			}

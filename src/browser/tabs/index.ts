...
class Tabs implements TabsInterface {
	...
	async saveSession() {
		const tabsCache = this.tabs.map((tab, index) => {
			// Prefer the live iframe URL (post-redirects, SPA navigation),
			// decoded through the central helper. Fall back to the original
			// creation URL — also run through the decoder defensively in case
			// it was already a proxied URL.
			const fromIframe = decodeIframeUrl(tab.iframe, this.proxy);
			const cleanUrl =
				fromIframe ||
				(tab.url ? decodeProxiedUrl(tab.url, this.proxy) : '');

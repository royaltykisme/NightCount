...
export class HistoryManager {
	...
	/**
	 * Returns the shared HistoryManager singleton.
	 * All callers within the renderer must use this to avoid OPFS write races
	 * (multiple TFS handles writing /data/history.json concurrently => EIO).
	 */
		...
		this.loadPromise = (async () => {
			try {
				...
				if (!this.currentSessionId) {
					...
					if (!existingSession) {
						this.currentSessionId =
							this.getOrCreateCurrentSession();
					}
				}
			} catch (error) {
	...
	/**
	 * Append a write task to the serial write queue. Tasks are awaited in order
	 * so we never have two concurrent writeFile() calls against history.json
	 * (which would deadlock/EIO on the OPFS sync access handle).
	 */
	...
	private async syncSessionsIfEnabled(): Promise<void> {
		...
		if (this.pendingSessionsSave) {
			return this.writeQueue;
		}
		this.pendingSessionsSave = true;
		return this.enqueueWrite(async () => {
			this.pendingSessionsSave = false;
			try {
	...
	public async addEntry(data: CreateHistoryEntryData): Promise<HistoryEntry> {
		...
		const existingEntry = this.entries.find(
			entry =>
				entry.url === url &&
				now.getTime() - entry.visitedAt.getTime() < 30000
		);
	...
	public async recordTabClose(tabId: string): Promise<void> {
		...
		if (startTime) {
			...
			const recentEntry = this.entries.find(
				entry =>
					entry.tabId === tabId &&
					Math.abs(entry.visitedAt.getTime() - startTime.getTime()) <
						5000
			);
		...
		switch (filter) {
			...
			case 'yesterday':
				return this.entries.filter(
					entry =>
						entry.visitedAt >= yesterday && entry.visitedAt < today
				);
			case 'week':
	...
	public searchEntries(query: string): HistorySearchResult[] {
		...
		this.entries.forEach(entry => {
			...
			const daysSinceVisit =
				(Date.now() - entry.visitedAt.getTime()) /
				(1000 * 60 * 60 * 24);
		...
		try {
			...
			if (data.entries) {
				...
				const existingUrls = new Set(
					this.entries.map(
						e => `${e.url}-${e.visitedAt.toISOString()}`
					)
				);
				const newEntries = importedEntries.filter(
					(entry: HistoryEntry) =>
						!existingUrls.has(
							`${entry.url}-${entry.visitedAt.toISOString()}`
						)
				);
				...
				if (data.sessions) {
					const importedSessions = data.sessions.map(
						(session: any) => ({
							...session,
							startTime: new Date(session.startTime),
							endTime: session.endTime
